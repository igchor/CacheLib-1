...
bool CacheAllocator<CacheTrait>::moveChainedItem(ChainedItem& oldItem,
  ...
  auto parentHandle = validateAndGetParentHandleForChainedMoveLocked(oldItem, parentKey);
...
void CacheAllocator<CacheTrait>::evictForSlabRelease(
  ...
  while (true) {
    ...
    const auto res = releaseBackToAllocator(*evicted, RemoveContext::kEviction, false);
    XDCHECK(res == ReleaseRes::kReleased);
    return;
  }
}
