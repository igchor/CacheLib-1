...
//   }
  ...
  void testBasicMultiTier() {
    ...
    /* Disable eviction and rebalancing so that after memory capacity is exhausted alloc will fail. */
    config.disableCacheEviction();
    config.enablePoolRebalancing(nullptr, std::chrono::milliseconds{0});
    ...
    /* Try to allocate as many elements as we can. */
    size_t allocatedBytes = 0;
    while(true) {
      std::string key = std::to_string(allocatedBytes);
      ...
      /// EXPECT_FALSE(alloc.allocator_[0]->isMemoryInAllocator(h->getMemory()));
    }

    /* Since multi-tier moving was enabled, we should have used more than just capacity of a
     * single tier. */
    EXPECT_GE(allocatedBytes, numBytes / 2);
    EXPECT_GE(movedKeys.size(), 0);
