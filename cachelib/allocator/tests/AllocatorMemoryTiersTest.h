 ...
 public:
  ...
  void testMultiTiersForceTierAllocation() {
...

    {
      ...
      ASSERT_NE(alloc.slabsFreePercentage(0), 100.0);
      ASSERT_EQ(alloc.slabsFreePercentage(1), 100.0);
    }
    ...
    config.forceAllocationTier = 1;
    {
      ...
      auto handle = alloc.allocate(pool, "key", std::string("value").size());
      ASSERT(handle != nullptr);
      ASSERT_EQ(alloc.slabsFreePercentage(0), 100.0);
      ASSERT_NE(alloc.slabsFreePercentage(1), 100.0);
    }
  }
  ...
  void testMultiTiersWatermarkAllocation() {
    ...
    config.sizeThresholdPolicy = 1000;
    {
      ...
      ASSERT_NE(alloc.slabsFreePercentage(0), 100.0);
      ASSERT_NE(alloc.slabsFreePercentage(1), 100.0);
      ASSERT_EQ(alloc.slabsFreePercentage(0), alloc.slabsFreePercentage(1));
    }
  }
};
} // namespace tests
