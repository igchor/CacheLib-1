...
#ifndef COROUTINE_HELPERS
#define COROUTINE_HELPERS
...
struct task {
  struct promise_type {
    ...
    template <typename R> void return_value(R&& result) { this->result = std::forward<R>(result);}
  ...
  bool await_ready() { return !h || h.done();}
    ...
    {
        return std::move(this->h.promise()).result;
    }
