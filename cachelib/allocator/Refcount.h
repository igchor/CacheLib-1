...
class FOLLY_PACK_ATTR RefcountWithFlags {
 public:
   ...
   * and item is not already marked for eviction nor moving and the ref count is
   * 0. This operation is atomic.
   *
   * Unmarking eviction does not depend on `isInMMContainer`
   */
   ...
   * is always >= 1 and `kExclusive` bit is set.
   *
 ...
 private:
   ...
   *
   * It atomically assigns result of newValueF(currentValue) to refCount_
   * only if predicate(currentValue) returns true.
   */
