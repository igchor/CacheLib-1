...
class FOLLY_PACK_ATTR RefcountWithFlags {
 public:
  ...
  static_assert(static_cast<uint8_t>(kMMFlag0) > static_cast<uint8_t>(kExclusive),
   ...
   *
   * An item can only be marked exclusive when `isInMMContainer` returns true and
   * the item is not yet marked as exclusive. This operation is atomic.
   *
  ...
  bool isExclusive() const noexcept { return getRaw() & getAdminRef<kExclusive>(); }
  bool isOnlyExclusive() const noexcept {
    // An item is only exclusive when its refcount is zero and only the exlusive bit
